...
def generatesign(model):
    ...
    nsamples = 2
    u_best = np.where(np.fabs(model.coef_[0]) > 1e-8, np.sign(model.coef_[0]), 0.0)
    u_sample = np.random.uniform(-1.5, 1.5, size=(nsamples, nparameter))
    u_sample = np.rint(u_sample)
...
#    print(u_sample)
    ...
    probold = np.square(model.predict_proba(u_sample)[:,1])
    norm = np.sum(probold)
    probold = probold/norm
    outindx = choices([i for i in range(nsamples)], weights=probold)[0]
    return u_sample[outindx,:]
#----------------------------------------------
def generatesign_markov(model):
    nparameter = len(model.coef_[0])
...
class LogisticSearch(ParameterData):
    ...
    '''
    #------------------------------------------------
    def __init__(self, parameters, lossfunction, lbounds, ubounds, depthscale=None, options={}, directmodel=None, distmodel=None, parentscale=1.0):
        ...
        if directmodel is None:
            self.distancemodel = LogisticRegression( max_iter=50000, class_weight='balanced' )
            self.directionmodel = LogisticRegression(max_iter=50000, class_weight='balanced')
            self.trained = False
            self.lasttrain = 0
            self.modelrmax = 0.0
        else:
...
#        return self.localshift(node=node)
        ...
        if oldsims < 3:
            ...
            try:
                direction = np.where(np.fabs(self.directionmodel.coef_[0]) > 1e-3, direction, 0.0) 
...
#        elif oldsims < 10:
            # Generates a vector in the gradient direction
#            try:
#                u = getgradient(self.parameters)
#                u = -u
#                rmax = min(1e-3, rmax)
#            except TypeError:
#                return self.localshift(node=node)
#                print("Gradient failed, Getting alternative")
#       :         u = np.random.uniform(-1.0, 1.0, size=len(self.parameters))
        else:
...
#        x_r = np.where(x_r < 0.0, 0.0, x_r)
    ...
    def logistic_localshift(self, node=None, curtrials=None):
        ...
        rmax = searchmax
        self.evalsince += 1
        if self.evalsince > 15 or (not self.trained):
            X = []
            Y = []
            R = []
            W = []
            ...
            for par, score in zip(playoutStruct, playoutEList):
                ...
                if score < self.score:
                    y = 1.0
                    try:
                        w = min(fabs(score - self.score)/fabs(self.score) + 1.0, 1.0)
                    except:
                        w = min(fabs(score - self.score) + 1.0, 1.0)
 #                   w = 1.0
                    lowerval = True
                    if bestscore > score:
                        bestscore = score
...
#                self.distancemodel.fit(r_train,Y,sample_weight = W )
                ...
                loss = self.directionmodel.score(X,Y)
                print("Direction Model Loss: %s"%(loss))
