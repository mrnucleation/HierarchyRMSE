...
class ParameterData(object):
    ...
    '''
    #------------------------------------------------
    def __init__(self, parameters, lossfunction, lbounds, ubounds, depthscale=None, parentscale=1.0, options={}):
        '''
         Input Variables:
        ...
        if 'customplayout' in options:
            self.customplayout = options['customplayout']
...
#            self.childub = np.array(self.childub)
    ...
    #----------------------------------------------------
    def applybounds(self, x_r_in):
        x_r = np.where(x_r_in > 1.0, 1.0, x_r_in)
        x_r = np.where(x_r < 0.0, 0.0, x_r)
        return x_r

    #----------------------------------------------------
    def setminimizer(self, evalfunc):
        self.minimizer = evalfunc
    #------------------------------------------------
    ...
    def runsim(self, playouts=1, node=None):
        ...
        if self.verbose:
            for playout, energy in enumerate(energylist):
                print("Playout %s Result: %s, Delta:%s"%(nPrev+playout+1, energy, energy-self.score))
...
#            nepoch = epochscale[-1]
        ...
        self.computechildbounds(node)
        score, newpar = self.minimizer(initialguess)
#        try:            
#        except:
#            score = minval
#            newpar = initialguess
    ...
    def minimize_simplex(self, node=None):
        ...
        searchmax = self.getsearchmax(node)
        playstructs, playoutEList = node.getplayouts()
        initialguess = np.copy(self.parameters)
        minval = node.getscore()
        for struct, eng in zip(playstructs, playoutEList):
            print(minval, eng)
            if eng < minval:
                minval = eng
                initialguess = np.copy(struct)
                print(eng)

        self.computechildbounds(node)
#        startstepsize=[(ub-lb)*searchmax for lb,ub in zip(self.lbounds, self.ubounds)]
        startstepsize=[(ub-lb)*0.1 for lb,ub in zip(self.lbounds, self.ubounds)]
        simplex = StephSimplex(self.lossfunction)
        lb = list(self.lbounds)
        ub = list(self.ubounds)
        initialguess = list(initialguess)
        results, score = simplex.runopt(lb, ub, initialguess=initialguess, maxeval=4000, 
                                        delmin=[1e-2 for x in self.lbounds], startstepsize=startstepsize)
        if self.verbose:
